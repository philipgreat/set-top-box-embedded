...
BOOL COverlay::LoadImageOntoSurface(LPDIRECTDRAWSURFACE7 lpdds, LPSTR lpstrResID)
{
    ...
    hbm = (HBITMAP)LoadImage(GetModuleHandle(NULL),
		MAKEINTRESOURCE(lpstrResID), IMAGE_BITMAP, 
		ddsd.dwWidth, ddsd.dwHeight, LR_COPYFROMRESOURCE|LR_CREATEDIBSECTION);

LR_LOADFROMFILE|LR_CREATEDIBSECTION
    hbm = (HBITMAP)::LoadBitmap(GetModuleHandle(NULL),MAKEINTRESOURCE(lpstrResID));

	*/
    hbm = (HBITMAP)LoadImage(NULL,
		lpstrResID, IMAGE_BITMAP, 
    ...
    else  //Looks like we're just using a standard RGB surface format, let GDI do the work.
    {
		...
		/*
        ::Rectangle(hdcImage,20,20,90,90);
		...
		rect.right = 200;
		rect.bottom=200;
		::DrawText(hdcImage,"100",3,&rect,DS_CENTER);
		*/
...
BOOL COverlay::DisplayOverlay()
{
	...
	int posx=scx/2-(600)/2;
	int posy=scy/2-(300)/2;
    rd.left=posx; rd.top=posy;
	
    rd.right  = (rs.right*uStretchFactor1000+999)/1000+posx; // adding 999 takes care of integer truncation problems.
    rd.bottom = rs.bottom*uStretchFactor1000/1000+posy;
...
BOOL COverlay::CreateOverlay()
{
    ...
    ddsdOverlay.dwWidth=600;
    ddsdOverlay.dwHeight=300;
...
int COverlay::ShowMenu(char *message, int percent)
{

	/*
	if (!LoadImageOntoSurface(m_lpddsOverlay, MAKEINTRESOURCE(IDB_BUGIMAGE1)))
        return FALSE;
		if (!DrawText(m_lpddsOverlay))
	{
		return FALSE;
	}
	*/
	...
	if (!this->SetBackgroudImage("c:\\showimage.bmp"))
        return FALSE;
	
	if (!Draw(m_lpddsOverlay))
	{
		return FALSE;
	}
...
int COverlay::Hide()
{
	...
	sprintf(filePath,"%s\\%s",currentDir,"hideimage.bmp");//,filePath;	
...
#include <stdio.h>
BOOL COverlay::Draw(LPDIRECTDRAWSURFACE7 lpdds)
{
    HBITMAP         hbm;
    HDC             hdcImage= NULL;
    HDC             hdcSurf = NULL;
	...
	sprintf(msg,
		"Build @ %s %s (C)2000-2006 Channel Soft(Beijing) ltd.",
	...
	*/
    ...
    else  //Looks like we're just using a standard RGB surface format, let GDI do the work.
    {
		...
		::DrawText(hdcImage,msg,strlen(msg),&rect,DT_CENTER);
...
BOOL COverlay::SetBackgroudImage(const char *fileName)
{
	this->imageFilePath = (char*)fileName;
	return TRUE;
}
