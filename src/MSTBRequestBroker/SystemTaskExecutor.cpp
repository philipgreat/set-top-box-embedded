...
static BOOL SuspendSystem(){
	...
	BOOL ret = 0;
    hInst=LoadLibrary("powrprof.dll");
    if(!hInst){	
		return FALSE;
	}
	
	SetSuspendState=(BOOL (*)(BOOL,BOOL,BOOL))GetProcAddress(hInst,"SetSuspendState");
...
static DWORD ExecuteFileEx(LPCTSTR currentDictory,LPCTSTR fileName,LPTSTR cmdLine)
{
	STARTUPINFO si;
	DWORD pid = 0;
...
bool CSystemTaskExecutor::IsProcessAlive(int processId)
{
	HANDLE hProcess;
	// Get the actual priority class.
